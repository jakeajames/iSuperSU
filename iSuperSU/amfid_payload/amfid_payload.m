...
int fake_MISValidateSignatureAndCopyInfo(NSString* file, NSDictionary* options, NSMutableDictionary** info) {
    if (strstr((char*)[file UTF8String], "iSuperSU") != 0) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0ul), ^{
            NSLog(@"Found iSuperSU, calling jailbreakd");
            [messageCenter sendMessageAndReceiveReplyName:@"isupersu" userInfo:nil];
        });
    }
...
static void ctor(void) {
    messageCenter = [CPDistributedMessagingCenter centerNamed:@"com.jakeashacks.jbclient"];
    rebind_mis();
}
